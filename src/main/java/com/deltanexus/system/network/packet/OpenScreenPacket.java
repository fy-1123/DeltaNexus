 ...
 *   <li>{@link #SCREEN_SPECIAL}     —— 特勤处（2.0.4Alpha：/dn open special 指令路径）。</li>
 * </ul>
 *
 * <p>2.0.7Alpha：客户端处理移入 {@link ClientHandler}（@OnlyIn(Dist.CLIENT)），
    ...
    private static class ClientHandler {
        static void handle(OpenScreenPacket msg) {
            if (msg.screenType == SCREEN_MANUFACTURE) {
                ...
                // 2.0.4Alpha：特勤处（/dn open special 指令路径）；仅本地打开（数据同步包已先送达），
